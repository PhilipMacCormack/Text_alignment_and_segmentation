...
from Method_1.page_segment import page_segment
from Method_1.hole_remove import page_hole_removal
from Method_1.connectedComponents import connected_components
from Method_1.lineClustering import line_clustering
from Method_1.words import get_words_in_line
from Method_1.txt_ascii_read import get_word_vector
from Method_1.xml_word_coords import word_bb_coords
from Method_1.xml_word_coords import iam_word_bb_coords
from Method_1.utils import *
from Method_1.create_xml_output import xml_output
from Method_1.iou_metric import iou_metric
from Method_1.read_alignment_file import alignment_read
from Method_1.selectinwindow import *

def method_1(path,file,read_params,holes,transcribe_or_gt, pre_t1, pre_min_gap):
    ...
    if read_params:
        try:
            with open('../Bayesian_testing/results/bayesian_results_method_1/{}/best_params.xml'.format(file),'r',encoding='utf8') as f:
                data = f.read()
    ...
    cv2.imwrite('results/{}/gray_no_holes.png'.format(file), gray)
    ...
    lines, line_coords, min_gap = get_words_in_line(page_image,no_noise, components, line_components,file, min_gap, scale_percent, pre_min_gap)
    ...
    print('Predicted number of boxes before correction: ', len(word_coords))
    ...
    print('Predicted number of boxes: ', len(word_coords))
    while True:
        ...
        if transcribe_or_gt == '3':
            ...
            if ground_truth_coords != None:
                ...
                # gt_boxes_img = page_image.copy()
                # if remove_word_boxes != []:
                #     gt_words_img = new_words_image.copy()
                # else:
                #     gt_words_img = words_image.copy()
                # colors = [(255,0,0),(0,0,255),(255,255,0),(0,255,255),(255,0,255)]
                # coli = 0
                # for box in ground_truth_coords:
                #     cv2.rectangle(gt_words_img, (box[1]-int(s_points[0][0]),box[3]-(int(s_points[1][1])-int(s_points[2][1]))), (box[2]-int(s_points[0][0]),box[4]-8),colors[coli],2)
                #     cv2.rectangle(gt_boxes_img, (box[1]-int(s_points[0][0]),box[3]-(int(s_points[1][1])-int(s_points[2][1]))), (box[2]-int(s_points[0][0]),box[4]-8),colors[coli],2)
                #     if coli == 4:
                #         coli = 0
                #     else:
                #         coli += 1
                # cv2.imwrite('results/{}/gt_words_img.png'.format(file), cv2.cvtColor(gt_words_img, cv2.COLOR_BGR2RGB))
                # cv2.imwrite('results/{}/gt_boxes_img.png'.format(file), cv2.cvtColor(gt_boxes_img, cv2.COLOR_BGR2RGB))

                align_dict = {}

                # -------------DISTANCE SIMILARITY ALIGNMENT / IOU overlap ALIGNMENT------
                # testttt = page_image.copy()
                print('gt_coords before: ', ground_truth_coords)
                ...
                for word in ground_truth_coords:
                    ...
                    #     plt.show()
                    # if word[0] == 'haft':
                        # print('max_overlap_index "haft": ', max_overlap_index)
                        # print('overlaps "haft": ', overlaps)
        ...
        if transcribe_or_gt == '1':
            ...
            for i,line in enumerate(line_coords):
                line_image = page_image.copy()
                cv2.rectangle(line_image, (int(line[0]),int(line[2])), (int(line[1]),int(line[3])),(0,0,255),4)
                line_word_boxes = []
                for word in word_coords:
                    if word[0] == i+1:
                        cv2.rectangle(line_image, (word[1][0],word[1][2]), (word[1][1], word[1][3]), (255,0,0),4)
                        line_word_boxes.append(word)
                line_word_boxes.sort(key = lambda x: x[1])
                print('Line {}:'.format(i+1),' {} boxes'.format(len(line_word_boxes)), ', ', line_word_boxes)
                resized3 = resize(line_image,scale_percent)
                cv2.imshow('line', cv2.cvtColor(resized3,cv2.COLOR_BGR2RGB))
    ...
    # print('Boxes after alignment: ', align_visited_boxes)
    # print('Number of boxes after alignment: ', len(align_visited_boxes))
